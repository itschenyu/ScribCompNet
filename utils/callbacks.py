...
class EvalCallback():
    ...
    def on_epoch_end(self, epoch, model_eval):
        if epoch % self.period == 0 and self.eval_flag:
            ...
            mious = np.array(self.mious)

            if max(mious) >= 80 and mious[-1] == max(mious):
                torch.save(self.net.state_dict(), os.path.join('logs', "best_epoch_weights_mious_%.3f.pth"%(mious[-1])))
